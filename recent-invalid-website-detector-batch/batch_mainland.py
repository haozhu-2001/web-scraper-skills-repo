...
import tempfile
import shutil
from references.db import query_one_mainland, update
...
def _decode_html(raw):
    """智能解码 HTML 字节：先探测 meta charset，再按 CJK 密度择优"""
    detected = None
    head = raw[:2048]
    ...
    candidates = [enc for enc in ([detected] if detected else []) if enc not in ("utf-8", "gbk", "gb18030")]
    ...
    if best_cjk == -1:
        best_html = raw.decode("latin-1", errors="replace")
    return best_html
...
SOFT_404_PATTERNS = [
    "未找到", "没有找到", "找不到", "不存在", "已删除",
    "已过期", "已下架", "已失效", "页面不存在", "抱歉",
    "not found", "no longer exists", "can't be found",
]
...
def main():
    ...
    print("=" * 60)
    print("Mainland 批量检测 — 逐条 curl + DB 更新")
    print("Ctrl+C 安全中断，自动释放占用态")
    print("=" * 60)

    while True:
        # 取一条未处理记录（自动设 finished=-1）
        row = query_one_mainland()
        if row is None:
            print("\n>>> 没有更多待处理记录，全部完成。")
            break
